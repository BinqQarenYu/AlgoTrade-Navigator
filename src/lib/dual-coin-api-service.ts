...
import { getTopCoins as getCoinGeckoCoins, getCoinDetailsByTicker } from './coingecko-service';
import { getCoinDetailsByTickerFromCMC } from './coinmarketcap-service';
import { getHistoricalKlines, getLatestKlinesByLimit } from './binance-service';
...
export class DualCoinApiService {
   ...
   * Uses available Binance and CoinGecko functions
   */
  private async executePriceRequest(ticker: string): Promise<{ price: number; change24h: number; volume24h: number } | null> {
    // Try Binance API first for real-time price data using latest klines
    if (this.canMakeRequest('binance')) {
      try {
        ...
        // Use getLatestKlinesByLimit to get recent price data
        const klineData = await getLatestKlinesByLimit(ticker, '1m', 1);
        if (klineData && klineData.length > 0) {
          const latestKline = klineData[0];
          this.incrementUsage('binance', 1);
          
          const price = latestKline.close;
          const prevClose = latestKline.open;
          const change24h = ((price - prevClose) / prevClose) * 100;
          const volume24h = latestKline.volume;
          
          console.log(`✅ Binance success for ${ticker}: $${price}`);
          return {
            price,
            change24h,
            volume24h
    ...
    // For CoinGecko price fallback, we'll need to use getCoinDetailsByTicker as there's no getCoinPrice
    if (this.canMakeRequest('coingecko')) {
      try {
        console.log(`📡 Fallback to CoinGecko for ${ticker}...`);
        
        // Wait for rate limit if needed
        await this.waitForRateLimit('coingecko');
        
        // Map ticker to coin symbol for CoinGecko
        const symbol = ticker.replace('USDT', '').toLowerCase();
        const coinDetails = await getCoinDetailsByTicker(symbol, this.coingeckoApiKey);
        
        if (coinDetails) {
          this.incrementUsage('coingecko');
          
          // Calculate price from market cap and circulating supply if available
          const price = coinDetails.marketCap / (coinDetails.circulatingSupply || 1);
          
          console.log(`✅ CoinGecko success for ${ticker}: $${price}`);
          return {
            price,
            change24h: coinDetails.priceChange24h || 0,
      ...
      console.log(`⏭️ CoinGecko not available (quota: ${apiUsage.coingecko.used}/${apiUsage.coingecko.limit})`);
    }
  ...
  async getHistoricalData(symbol: string, interval: string = '1h', limit: number = 100): Promise<any[] | null> {
    ...
    if (this.canMakeRequest('binance')) {
      try {
        ...
        const klines = await getLatestKlinesByLimit(symbol, interval, limit);
