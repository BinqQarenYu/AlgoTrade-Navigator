...
const defaultParams: HyperPFFParams = {
    peakLookaround: 5,
    swingLookaround: 3,
    emaShortPeriod: 13,
    emaLongPeriod: 50,
...
export async function calculateHyperPeakFormationLogic(data: HistoricalData[], params: HyperPFFParams): Promise<HistoricalData[]> {
                ...
                for (let k = i + 1; k < Math.min(i + params.maxLookahead, data.length); k++) {
                    ...
                    if (emaCrossed && structureBroken) {
                        ...
                        for (let l = bosIndex + 1; l < Math.min(bosIndex + params.maxLookahead, data.length); l++) {
                            ...
                            const fibLevel1 = pullbackLow + fibRange * params.fibLevels[0];
                            const fibLevel2 = pullbackLow + fibRange * params.fibLevels[1];
                ...
                for (let k = i + 1; k < Math.min(i + params.maxLookahead, data.length); k++) {
                    ...
                    if (emaCrossed && structureBroken) {
                        ...
                        for (let l = bosIndex + 1; l < Math.min(bosIndex + params.maxLookahead, data.length); l++) {
                            ...
                            const fibLevel1 = pullbackHigh - fibRange * params.fibLevels[0];
                            const fibLevel2 = pullbackHigh - fibRange * params.fibLevels[1];
...
const hyperPeakFormationStrategy: Strategy = {
    ...
    async calculate(data: HistoricalData[]): Promise<HistoricalData[]> {
        return calculateHyperPeakFormationLogic(data, defaultParams);
    }
}
