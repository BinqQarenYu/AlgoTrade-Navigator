...
const codeBasedConsensusStrategy: Strategy = {
  ...
  async calculate(data: HistoricalData[], params: CodeBasedConsensusParams = defaultCodeBasedConsensusParams): Promise<HistoricalData[]> {
    const dataWithIndicators = data.map((d: any) => ({ ...d })); // ⚡ Bolt: Shallow copy for performance, avoids slow JSON deep cloning
    ...
    for (const strategyId of params.strategies) {
        ...
        if (strategy) {
            ...
            allStrategyCalculations.push(strategy.calculate(data.map((d: any) => ({ ...d })))); // ⚡ Bolt: Shallow copy for performance, avoids slow JSON deep cloning
        }
    }
