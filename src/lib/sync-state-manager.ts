...
/**
 * Returns the gap that needs to be filled for a symbol.
 * The gap is between the lookback target and the oldest already-fetched timestamp.
 * Returns null if backfill is already complete.
 */
export function getGap(
  symbol: string,
  lookbackMs: number = 60 * 24 * 60 * 60 * 1000 // 2 months default
  ...
  // No data at all for this symbol — full backfill needed
  if (!state) {
  ...
  // Gap is from target start to oldest fetched timestamp
  ...
  return null; // Nothing left to fetch
}

/**
 * Update manifest after a chunk is successfully saved.
 */
...
/**
 * Get the timeline for UI display — returns covered ranges and gaps.
 */
export function getTimeline(
  ...
  const coveredRange = state.newestFetchedTimestamp - state.oldestFetchedTimestamp;
