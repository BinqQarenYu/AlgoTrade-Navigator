 ...
 * Supports an optional accessor function to work directly on objects without pre-allocating number arrays.
 ...
 * @param getValue Optional accessor function to extract numeric value from an item
...
export const calculateDonchianChannels = (data: HistoricalData[], period: number): { upper: (number | null)[], middle: (number | null)[], lower: (number | null)[] } => {
  // Pass property accessors directly to eliminate intermediate data.map() array allocations
...
export const calculateWilliamsR = (data: HistoricalData[], period: number): (number | null)[] => {
    ...
    const len = data.length;
    const williamsR: (number | null)[] = new Array(len);
    for (let i = 0; i < len; i++) {
        const highestHigh = periodHighs[i];
        const lowestLow = periodLows[i];
