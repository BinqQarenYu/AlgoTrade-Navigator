...
export function EntropyHUD({ orderFlowData }: EntropyHUDProps) {
  ...
  const metrics = useMemo(() => {
    ...
    // ⚡ Bolt Optimization: Replace 5 chained O(N) mutations (.reduce, .filter().reduce())
    // with a single O(N) iterative loop to drastically reduce GC overhead during frequent React renders.
    let totalEntropy = 0;
    let totalToxicity = 0;
    let toxicTraps = 0;
    let buys = 0;
    ...
    for (let i = 0; i < window.length; i++) {
        ...
        if (o.microstructure) {
            totalEntropy += o.microstructure.entropyScore || 0;
            totalToxicity += o.microstructure.vpin || 0;
            if (o.microstructure.isToxicTrap) toxicTraps++;
            if (!latest) latest = o.microstructure;
        }
