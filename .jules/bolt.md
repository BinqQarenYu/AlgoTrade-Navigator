...
## 2026-03-29 - Single-Pass Sliding Window Optimization for Awesome Oscillator (AO)
**Learning:** `calculateAwesomeOscillator` pre-allocated multiple $O(N)$ arrays (`data.map` for median prices, `calculateSMA` for short SMA, `calculateSMA` for long SMA, and `smaShort.map`) causing 4 separate array allocations and passes. Calculating median price on-the-fly inside a single sliding window loop with rolling sums eliminates 3 array allocations and reduces runtime GC pressure.
**Action:** Replace `data.map()` and separate `calculateSMA()` calls in `calculateAwesomeOscillator` with a single-pass loop maintaining running `sumShort` and `sumLong` variables.
